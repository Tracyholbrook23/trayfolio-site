...
/* eslint-disable react-hooks/set-state-in-effect, react-hooks/refs --
   Vendored as-is from 21st.dev/Motiq (compare-reveal). It drives an
   imperative rAF spring loop and intentionally mutates refs during
   render to seed that loop with the latest props/state without an
   extra effect tick; rewriting it risks changing the drag physics. */
...
/* Compare Reveal — from Motiq (https://motiq.dev/components/compare-reveal).
   MIT licensed. Zero runtime dependencies. Pulled in via the 21st.dev MCP
   and dropped in as-is; only the calling site (SeeTheDifference.tsx)
   overrides the --motiq-* color tokens to match the Trayfolio palette. */
